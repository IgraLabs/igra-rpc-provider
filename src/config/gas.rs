...
impl GasConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.min_protocol_fee_per_gas_gwei > MAX_REASONABLE_PROTOCOL_FEE_GWEI {
            return Err(format!(
                "Minimum protocol fee per gas is unreasonably high: {} gwei (max reasonable: {} gwei)",
                self.min_protocol_fee_per_gas_gwei, MAX_REASONABLE_PROTOCOL_FEE_GWEI
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_gas_config_requires_explicit_value() {
        // Test that GasConfig cannot be created without explicit configuration
        // This is a compile-time check - the struct has no Default implementation
    ...
    fn test_gas_config_validation_too_high() {
        let config =
            GasConfig::with_min_protocol_fee_per_gas_gwei(MAX_REASONABLE_PROTOCOL_FEE_GWEI + 1);
        assert!(config.validate().is_err());
        assert!(config
            .validate()
            .expect_err("Expected validation to fail")
